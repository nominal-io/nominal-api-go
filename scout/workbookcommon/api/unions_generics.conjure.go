...
type WorkbookStatsPanelScopeWithT[T any] WorkbookStatsPanelScope

func (u *WorkbookStatsPanelScopeWithT[T]) Accept(ctx context.Context, v WorkbookStatsPanelScopeVisitorWithT[T]) (T, error) {
	var result T
	switch u.typ {
	default:
		if u.typ == "" {
	...
	case "selectedTab":
		if u.selectedTab == nil {
			return result, fmt.Errorf("field \"selectedTab\" is required")
		}
		return v.VisitSelectedTab(ctx, *u.selectedTab)
	case "allTabs":
		if u.allTabs == nil {
			return result, fmt.Errorf("field \"allTabs\" is required")
		}
		return v.VisitAllTabs(ctx, *u.allTabs)
	}
}

func (u *WorkbookStatsPanelScopeWithT[T]) AcceptFuncs(selectedTabFunc func(SelectedTabStatsPanelScope) (T, error), allTabsFunc func(AllTabsStatsPanelScope) (T, error), unknownFunc func(string) (T, error)) (T, error) {
	var result T
	switch u.typ {
	default:
		if u.typ == "" {
...
func (u *WorkbookStatsPanelScopeWithT[T]) SelectedTabNoopSuccess(SelectedTabStatsPanelScope) (T, error) {
	var result T
	return result, nil
}

func (u *WorkbookStatsPanelScopeWithT[T]) AllTabsNoopSuccess(AllTabsStatsPanelScope) (T, error) {
	var result T
	return result, nil
}

func (u *WorkbookStatsPanelScopeWithT[T]) ErrorOnUnknown(typeName string) (T, error) {
	var result T
	return result, fmt.Errorf("invalid value in union type. Type name: %s", typeName)
}

type WorkbookStatsPanelScopeVisitorWithT[T any] interface {
	VisitSelectedTab(ctx context.Context, v SelectedTabStatsPanelScope) (T, error)
	VisitAllTabs(ctx context.Context, v AllTabsStatsPanelScope) (T, error)
	VisitUnknown(ctx context.Context, typ string) (T, error)
}
