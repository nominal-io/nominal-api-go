...
type CanvasConnectionV1 struct {
	...
	*/
	...
	Routing *CanvasConnectionRouting `json:"routing,omitempty"`
}
...
/*
A right-angle route. Each entry is the fixed coordinate of one interior
segment, in logical canvas coordinates: x for vertical segments, y for
horizontal ones. Entries alternate axis, starting perpendicular to the stub
of the lexicographically smaller (objectId, anchorId) endpoint, and are
independent of arrow direction. Values must be finite.
*/
type OrthogonalConnectionRouting struct {
	SegmentPositions []float64 `json:"segmentPositions"`
}

func (o OrthogonalConnectionRouting) MarshalJSON() ([]byte, error) {
	if o.SegmentPositions == nil {
		o.SegmentPositions = make([]float64, 0)
	}
	type _tmpOrthogonalConnectionRouting OrthogonalConnectionRouting
	return safejson.Marshal(_tmpOrthogonalConnectionRouting(o))
}

func (o *OrthogonalConnectionRouting) UnmarshalJSON(data []byte) error {
	type _tmpOrthogonalConnectionRouting OrthogonalConnectionRouting
	var rawOrthogonalConnectionRouting _tmpOrthogonalConnectionRouting
	if err := safejson.Unmarshal(data, &rawOrthogonalConnectionRouting); err != nil {
		return err
	}
	if rawOrthogonalConnectionRouting.SegmentPositions == nil {
		rawOrthogonalConnectionRouting.SegmentPositions = make([]float64, 0)
	}
	*o = OrthogonalConnectionRouting(rawOrthogonalConnectionRouting)
	return nil
}

func (o OrthogonalConnectionRouting) MarshalYAML() (interface{}, error) {
	jsonBytes, err := safejson.Marshal(o)
	if err != nil {
		return nil, err
	}
...
func (o *OrthogonalConnectionRouting) UnmarshalYAML(unmarshal func(interface{}) error) error {
	jsonBytes, err := safeyaml.UnmarshalerToJSONBytes(unmarshal)
	if err != nil {
		return err
	}
