...
type CanvasConnectionRoutingWithT[T any] CanvasConnectionRouting

func (u *CanvasConnectionRoutingWithT[T]) Accept(ctx context.Context, v CanvasConnectionRoutingVisitorWithT[T]) (T, error) {
	var result T
	switch u.typ {
	default:
		if u.typ == "" {
	...
	case "orthogonal":
		if u.orthogonal == nil {
			return result, fmt.Errorf("field \"orthogonal\" is required")
		}
		return v.VisitOrthogonal(ctx, *u.orthogonal)
	}
}

func (u *CanvasConnectionRoutingWithT[T]) AcceptFuncs(orthogonalFunc func(OrthogonalConnectionRouting) (T, error), unknownFunc func(string) (T, error)) (T, error) {
	var result T
	switch u.typ {
	default:
		if u.typ == "" {
...
func (u *CanvasConnectionRoutingWithT[T]) OrthogonalNoopSuccess(OrthogonalConnectionRouting) (T, error) {
	var result T
	return result, nil
}

func (u *CanvasConnectionRoutingWithT[T]) ErrorOnUnknown(typeName string) (T, error) {
	var result T
	return result, fmt.Errorf("invalid value in union type. Type name: %s", typeName)
}

type CanvasConnectionRoutingVisitorWithT[T any] interface {
	VisitOrthogonal(ctx context.Context, v OrthogonalConnectionRouting) (T, error)
	VisitUnknown(ctx context.Context, typ string) (T, error)
}
