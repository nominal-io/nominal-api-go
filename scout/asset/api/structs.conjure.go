...
type Asset struct {
	...
	*/
	...
	// The dataset that backs this asset's data. Present when created with datasetRid. May be absent for legacy assets without representable scopes.
...
type CreateAssetRequest struct {
	Title string `json:"title"`
	/*
	   Dataset to back this asset. Must be readable and in the asset's workspace.
	   Composition datasets attach directly and must not already back another asset.
	   Raw datasets are wrapped in a new composition dataset; the source remains unchanged.
	   The response datasetRid identifies the backing dataset, which may differ from this RID.
	   Mutually exclusive with nonempty dataScopes. Omit to use legacy scope-based creation.
	*/
	DatasetRid  *rids.DatasetRid `json:"datasetRid,omitempty" safelogging:"@Safe"`
	Description *string          `json:"description,omitempty"`
	// Deprecated: use typedProperties
