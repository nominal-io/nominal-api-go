...
type NumRangesConditionV3 struct {
	...
	*/
	ComputeExpression *ComputeExpression `json:"computeExpression,omitempty"`
	/*
	   Default fill strategy applied at every multi-input alignment boundary in this check's
	   compute graph where no per-node interpolationConfiguration is specified, resolved to
	   the compute API fill strategy at execution time. Must be identical across every check
	   in the checklist; create and update reject checklists whose checks disagree. When
	   absent, falls back to the server-configured default (forward fill with a 1 second
	   fill limit).
	*/
	DefaultFillStrategy *CheckFillStrategy `json:"defaultFillStrategy,omitempty"`
	/*
	   Default alignment strategy applied at every multi-input alignment boundary in this
	   check's compute graph where no per-node alignment strategy is specified, resolved to
	   the compute API alignment at execution time. Must be identical across every check in
	   the checklist; create and update reject checklists whose checks disagree. When
	   absent, falls back to the server default (driver-series alignment).
	*/
	DefaultAlignmentStrategy *CheckAlignmentStrategy `json:"defaultAlignmentStrategy,omitempty"`
}
...
type ParameterizedNumRangesConditionV1 struct {
	...
	*/
	...
	/*
	   Default fill strategy applied at every multi-input alignment boundary in this check's
	   compute graph, shared by every implementation, where no per-node
	   interpolationConfiguration is specified, resolved to the compute API fill strategy at
	   execution time. Must be identical across every check in the checklist; create and
	   update reject checklists whose checks disagree. When absent, falls back to the
	   server-configured default (forward fill with a 1 second fill limit).
	*/
	DefaultFillStrategy *CheckFillStrategy `json:"defaultFillStrategy,omitempty"`
	/*
	   Default alignment strategy applied at every multi-input alignment boundary in this
	   check's compute graph, shared by every implementation, where no per-node alignment
	   strategy is specified, resolved to the compute API alignment at execution time. Must
	   be identical across every check in the checklist; create and update reject checklists
	   whose checks disagree. When absent, falls back to the server default (driver-series
	   alignment).
	*/
	DefaultAlignmentStrategy *CheckAlignmentStrategy `json:"defaultAlignmentStrategy,omitempty"`
}
